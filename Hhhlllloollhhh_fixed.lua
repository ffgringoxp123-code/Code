...
-- Fallback an toàn: nếu executor không hỗ trợ các hàm này, dùng bản thay thế
-- thay vì để script lỗi và dừng giữa chừng (khiến UI không bao giờ hiện ra).
...
if not get_remote_not then
    local retry_count = 0
    while not get_remote_not and retry_count < 15 do
        retry_count = retry_count + 1
        task.wait(math.min(0.5 + retry_count * 0.3, 3))
        runGetgc()
    end
end
...
function System.parry.execute()
    ...
    if Alive then
        for _, entity in pairs(Alive:GetChildren()) do
            if entity.PrimaryPart then
                local success2, screen_point = pcall(function() return camera:WorldToScreenPoint(entity.PrimaryPart.Position) end)
                if success2 then
    ...
    pcall(function()
        if get_remote_not then
            get_remote_not:FireServer(
                ...
                (function()
                    local get_t_net = ""
                    for i = 1, #tostring(math.floor(workspace:GetServerTimeNow() * 100)) do
...
local function IsEligibleToSpam()
    ...
    if not char or not char:FindFirstChildOfClass("Humanoid") or char:FindFirstChildOfClass("Humanoid").Health <= 0 then return false end
    local inMatch = player:GetAttribute("InMatch")
    local isPlaying = player:GetAttribute("IsPlaying")
    if inMatch == false or isPlaying == false then return false end
    return true
end
...
local function GetParryAnimation()
    ...
    local success, swordData = pcall(function() return game:GetService("ReplicatedStorage").Shared.ReplicatedInstances.Swords.GetSword:Invoke(currentSword) end)
...
game:GetService("ReplicatedStorage").Remotes.ParrySuccess.OnClientEvent:Connect(function()
    ...
    if not spam_started then
        ...
        if clash_count >= 6 then spam_started = true end
    end
...
function System.manual_spam.start()
    ...
    System.__properties.__connections.__manual_spam_connection = RunService.RenderStepped:Connect(function()
        if not System.__properties.__manual_spam_enabled or not IsEligibleToSpam() then return end
        local now = os.clock()
        ...
        local cooldown = 1.3
        if spam_started then cooldown = 0.08 end
...
function System.auto_spam.start()
    ...
    System.__properties.__connections.__auto_spam_connection = RunService.Heartbeat:Connect(function()
        local char = LocalPlayer.Character
        ...
        if ball_properties and entity_properties then
            ...
            local spam_accuracy = System.auto_spam:spam_service({ Ball_Properties = ball_properties, Entity_Properties = entity_properties, Ping = ping_threshold })
            if spam_accuracy and spam_accuracy > 0 then
...
local function create_mobile_button(text, transparency, color)
    ...
    label.Parent = frame
    return { button = frame, text = label, gui = screenGui }
end
    ...
    callback = function(value)
        ...
        if value then
            ...
            if getgenv().AutoParryNotify then
                main.SendNotification({ title = "Auto Parry", text = "ON", duration = 2 })
            end
        else
            System.autoparry.stop()
            if getgenv().AutoParryNotify then
                main.SendNotification({ title = "Auto Parry", text = "OFF", duration = 2 })
            end
        end
    end
})
    ...
    callback = function(value)
        ...
        if value then
            ...
            if getgenv().AutoSpamNotify then
                main.SendNotification({ title = "Auto Spam", text = "ON", duration = 2 })
            end
        else
            System.auto_spam.stop()
            if getgenv().AutoSpamNotify then
                main.SendNotification({ title = "Auto Spam", text = "OFF", duration = 2 })
            end
        end
    end
})
...
local manual_spam_module = rage:create_module({
    ...
    callback = function(state)
        if System.__properties.__is_mobile then
                    ...
                    manual_spam_mobile.button.InputEnded:Connect(function(input)
                        if input.UserInputType == Enum.UserInputType.Touch and not manual_was_dragged then
                            ...
                            if getgenv().ManualSpamNotify then
                                main.SendNotification({ title = "ManualSpam", text = System.__properties.__manual_spam_enabled and "ON" or "OFF", duration = 2 })
                            end
                        end
                    end)
                end
            ...
            if getgenv().ManualSpamNotify then
                main.SendNotification({ title = "Manual Spam", text = state and "ON" or "OFF", duration = 2 })
            end
        end
    end
})
