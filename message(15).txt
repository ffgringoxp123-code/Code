...
local function initialize(Library)
    ...
    AntiAimGroupbox:AddSlider("anti_aim_custom_pitch", { Text = "pitch angle", Min = -90, Max = 90, Default = 0, Rounding = 0 })
    ...
    AtmoGroupbox:AddToggle("atmosphere_enabled", { Text = "enable", Default = false })
        :AddColorPicker("atmosphere_color", { Default = Color3.new(0.784, 0.863, 1) })
    ...
    Options.aimbot_fov_rotation_speed:OnChanged(function()
    ...
    local function updateFOV()
        if Toggles.camera_fov_changer.Value then
            workspace.CurrentCamera.FieldOfView = Options.camera_aspect_ratio_y.Value * 70
        else
            workspace.CurrentCamera.FieldOfView = defaultFOV
        end
    end
    Toggles.camera_fov_changer:OnChanged(updateFOV)
    Options.camera_aspect_ratio_y:OnChanged(updateFOV)

    -- Shoot sound speed
    local function applyShootSoundSpeed()
        local speed = Options.shoot_sound_speed.Value
        pcall(function()
            for _, sound in ipairs(workspace:GetDescendants()) do
                if sound:IsA("Sound") and (sound.Name:lower():find("shoot") or sound.Name:lower():find("fire") or sound.Name:lower():find("shot")) then
                    sound.PlaybackSpeed = speed
                end
            end
        end)
    end
    Options.shoot_sound_speed:OnChanged(applyShootSoundSpeed)

    -- Target highlight
    local highlightInstance = nil
    local function updateTargetHighlight()
        if not Toggles.target_highlight_fill.Value then
    ...
    Toggles.target_highlight_fill:OnChanged(updateTargetHighlight)

    RunService.RenderStepped:Connect(function()
        if not Toggles.target_highlight_fill.Value then return end
        ...
        if closestChar then
            if not highlightInstance or not highlightInstance.Parent then
                highlightInstance = Instance.new("SelectionBox")
                highlightInstance.LineThickness = 0.05
                highlightInstance.SurfaceTransparency = 1 - (1 - Options.target_highlight_color.Transparency)
                highlightInstance.Color3 = Options.target_highlight_color.Value
                highlightInstance.Parent = workspace
            end
            highlightInstance.Adornee = closestChar
            highlightInstance.Color3 = Options.target_highlight_color.Value
            highlightInstance.SurfaceColor3 = Options.target_highlight_color.Value
            highlightInstance.SurfaceTransparency = Options.target_highlight_color.Transparency
        else
            if highlightInstance then
    ...
    Toggles.chat_spam_enabled:OnChanged(function()
        if chatSpamConn then chatSpamConn:Disconnect(); chatSpamConn = nil end
        if not Toggles.chat_spam_enabled.Value then return end
    ...
    RunService.Heartbeat:Connect(function()
        if not Toggles.arcade_auto_ammo.Value then return end
        pcall(function()
            for _, obj in workspace:GetDescendants() do
                if obj:IsA("BasePart") and obj.Name:lower():find("ammo") then
                    local dist = (LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart"))
                        and (obj.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude or math.huge
                    if dist < 20 then
                        local remote = game:GetService("ReplicatedStorage"):FindFirstChild("Remotes")
                        if remote then
                            local pickup = remote:FindFirstChild("PickupAmmo") or remote:FindFirstChild("Pickup")
                            if pickup then pcall(function() pickup:FireServer(obj) end) end
                        end
                    end
                end
            end
    ...
    -- Selected player view (spectate)
    RunService.RenderStepped:Connect(function()
        if not Toggles.selected_player_view.Value then return end
        pcall(function()
            local screenCenter = workspace.CurrentCamera.ViewportSize / 2
            local closestDist = Options.silent_radius.Value
            local closestChar = nil
            ...
            if closestChar then
                workspace.CurrentCamera.CameraSubject = closestChar:FindFirstChildOfClass("Humanoid") or closestChar:FindFirstChild("HumanoidRootPart")
            end
        end)
    end)
    Toggles.selected_player_view:OnChanged(function()
        if not Toggles.selected_player_view.Value then
            pcall(function()
